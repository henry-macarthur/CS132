...
//when we use a parameter need to make sure we look in the object class type not the actual class type
//need t0
...
class Visitor extends GJDepthFirst<String, Context> {
    ...
    public String visit(Identifier n, Context c) {
        ...
        String res = "t" + k + " = " + n.f0.tokenImage + "\n";
        ...
        //need to get the class of the object we are in, so current class should refer to the this object 
        //if its a message send we can set the current class to the type of the object 
        ...
        // else if (current_state == State.SEND && !current_class.equals("main")  && c.containsSubstr(c.class_to_methods.get(current_class), n.f0.tokenImage) != -1) {
        //     int index = c.containsSubstr(c.class_to_methods.get(current_class), n.f0.tokenImage);
        //     String get_table = "t" + (k+1) + " = [" + "this" + " + 0]\n"; 
        //     String get_function = "t" + (k) + " = [t" + (k+1) + " + " + (4 * index) + "]\n";
        //     res = get_table + get_function;
        // }
        k += 2;
    ...
    public String visit(AssignmentStatement n, Context c) {
        ...
        String assignment = n.f0.f0.tokenImage + " = " + "t" + k_old + "\n";
        ...
        if(current_state == State.METHOD && !c.method_to_parameters.get(current_method).contains(n.f0.f0.tokenImage)) {
            ...
            //System.out.println("!!");
            //have to look through parent data names and try 
            String tmp_class = current_class; 
            // while(c.class_to_parameters.containsKey(tmp_class)) {
            //     tmp_class = c.class_to_parent.get(tmp_class);
            // }
            int index = c.class_to_parameters.get(current_class).indexOf(parameter_name);
    ...
    */
    ...
    public String visit(AllocationExpression n, Context c) {
        ...
        //String parameter_name = current_class + "-" + id_name;
        //System.out.println("!!");
        for(int i = 0; i < c.class_to_parameters.get(allocated_type).size(); i++) {
            num_vars++;
        }
    ...
    public String visit(MessageSend n, Context c) {
        ...
        String id_primary = primary_expr.substring(primary_expr.indexOf("=") + 2, primary_expr.length() - 1);
        ...
        String together = primary_expr + get_table + get_func; //this gives me the function to call, now its time to call it!
        ...
        //     allocated_type = id_to_type.get(allocated_type);
        // }
        // class_type = allocated_type;
        // current_state = prev_state;
        // declarations += primary_expr;
        // int midk = k;
        // current_state = State.SEND;
        ...
        // String load_table = "";
        // String callt = "" + (originalk + 1);

        // if(method_name.contains("[t")) {
        //     load_function = method_name;
        //     callt = "" + (midk);
        // } else {
        //     System.out.println("ASDF");
        //     load_table = "t" + originalk + " = [t" + oldk + " + 0]\n";
        //     int index = 0;
        //     //need to have a way to get the type so we know which table to look into for methods!

        //     method_name = method_name.substring(0, method_name.length() - 1);
        //     method_name = method_name.substring(method_name.indexOf("=") + 2);

        //     for(int i = 0; i < c.class_to_methods.get(class_type).size(); i++) {

        //         int indexOfDash = c.class_to_methods.get(class_type).get(i).indexOf("_") + 1;

        //         if(c.class_to_methods.get(class_type).get(i).substring(indexOfDash).equals(method_name)) {
        ...
        //     load_function = "t" + (originalk + 1) + " = [t" + originalk + " + " + (index * 4) + "]\n";
        // }

        // //have to go and accept all of the parameter expressions
        // //old k has the id we want to call
        
        // String expr_list = "";
        // if(n.f4.present())
        //     expr_list = n.f4.accept(this, c);

        // String parameters = "t" + oldk;
        // //System.out.println("************");
        // System.out.println(primary_expr);
        // System.out.println(expr_list);
        // System.out.println(load_function);
        // System.out.println(load_table);
        // System.out.println("************!");
        // if(!expr_list.equals("")) {
        //     parameters += " " + expr_list.substring(expr_list.indexOf("this t") + 5);
        // }
        ...
        // expr_list = declarations + expr_list + load_table + load_function;
        // String send = "t" + originalk + " = " + "call t" + callt + "(" + parameters + ")\n";
        
        // return expr_list + send; 
        return together;
    }
